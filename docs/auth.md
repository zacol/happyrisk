...
2.  **Refresh Token (Opaque String):**
    ...
    - **Storage:** Handled strictly via an `HttpOnly`, `Secure`, `SameSite=Strict`. It is never exposed to frontend JavaScript.
...
- _Example:_ Only users with `UserRole.ADMIN` can access global settings endpoints.
